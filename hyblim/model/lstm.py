...
class LSTMCell(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, T_max: int = -1):
        super().__init__()
...
class EncDecLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.processor["encoder"] = nn.ModuleList(
            [LSTMCell(self.hidden_dim, self.hidden_dim, T_max=self.T_max) for i in range(self.num_layers)]
        )
        # Decoder
        ...
        self.processor["to_data"] = TailEnsemble(self.hidden_dim, self.input_dim, num_tails=self.num_tails)

        # FilM layer
        if self.use_film:
            self.processor["embedding"] = nn.Embedding(num_embeddings=num_conditions, embedding_dim=8 * self.hidden_dim)
...
class ResidualLSTM(nn.Module):
    ...
    def __init__(
        self, input_dim: int, hidden_dim: int = 64, num_layers: int = 1, num_conditions: int = -1, T_max: int = 5
    ) -> None:
        super().__init__()
