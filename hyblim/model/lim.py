...
def integrate_lim(L, noise_cov, x_0=None, n_times=100, dt=1):
    ...
    for i in range(n_times):
        Y.append(
            Y[i - 1]
            + L @ Y[i - 1] * dt_int
            + np.random.multivariate_normal(np.zeros(n_components), cov=noise_cov) * np.sqrt(dt_int)
        )
    Y = np.array(Y)
    # X(t+dt/2) = (Y(t) + Y(t+dt))/2
...
class LIM:
    ...
    def noise_covariance(self):
        ...
        if np.min(w) < 0:
            print("WARNING: Q has negative eigenvalues! Rescale Q!")
    ...
    def euler_integration(self, x0: np.ndarray, dt: float, T: int, num_samples: int = 1) -> tuple:
        ...
        for n in range(num_samples):
            ...
            for i in range(num_steps):
                ...
                X[n, i + 1, :] += np.random.multivariate_normal(np.zeros(num_components), cov=self.Q) * np.sqrt(dt)
    ...
    def save(self, filename, **kwargs):
        ...
        """
        store_dict = dict(tau=self.tau_0, G=self.G, L=self.L, Q=self.Q)
...
class CSLIM:
    ...
    def rollout_mean(self, x_init: np.ndarray, init_month: int, lag_arr: np.ndarray) -> np.ndarray:
        """Rollout forecast for given lags.
    ...
    def euler_integration(self, x0: np.ndarray, month0: int, dt: float, T: int, num_samples: int = 1) -> tuple:
        """Solve the SDE using the Euler method.
        ...
        for n in range(num_samples):
            ...
            for i, t in enumerate(times[:-1]):
                ...
                X[n, i + 1, :] += np.random.multivariate_normal(np.zeros(num_components), cov=Q) * np.sqrt(dt)
