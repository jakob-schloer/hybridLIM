        ...
        self.tails = nn.ModuleList(
            [
                nn.Sequential(
                    ...
                    nn.ConvTranspose3d(
                        patch_dim, output_dim, kernel_size=patch_size, stride=patch_size, groups=output_dim
                    ),
                )
...
class EncDecSwinLSTM(nn.Module):
    def __init__(
        ...
        self.processor["to_latent"] = nn.Conv3d(
            input_dim, num_channels, kernel_size=(1, *patch_size), stride=(1, *patch_size)
        )
        ...
        self.processor["to_data"] = TailEnsemble(num_channels, output_dim, (1, *patch_size), num_tails)

        if self.use_film:
            self.processor["embedding"] = nn.Embedding(num_embeddings=num_conditions, embedding_dim=8 * num_channels)
    ...
    def forward(self, x: th.Tensor, context: th.Tensor):
        ...
        h = [
            th.zeros(
                (batch, self.num_channels, height // self.patch_size[0], width // self.patch_size[1]), device=x.device
            )
