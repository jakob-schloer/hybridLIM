...
class TimeSeriesResidual(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample["time"] = preproc.time2timestamp(self.data.isel(time=ids_target)["time"].data)
        ...
        return (
            sample["lim"],
            sample["target"],
            {"time": sample["time"], "idx": sample["idx"], "month": sample["month"], "lag": sample["lag"]},
        )
...
class SpatialTemporalData(Dataset):
    ...
    def __init__(self, data: xr.Dataset, n_timesteps: int, transform=None):
        super().__init__()
    ...
    def torch2xr(self, data: torch.Tensor) -> xr.Dataset:
        ...
        if len(data.shape) == 4:
            ...
            for i, var in enumerate(self.vars):
                ...
                coords = {"time_idx": time_idx, "lat": self.data.coords["lat"], "lon": self.data.coords["lon"]}

                da_arr.append(xr.DataArray(data=sample_map, coords=coords, name=var))
        ...
        # For batch
            ...
            for sample in data:
                ...
                for i, var in enumerate(self.vars):
                    ...
                    da_arr.append(xr.DataArray(data=data_map, coords=coords, name=var))
...
def load_pcdata(
    ...
    **kwargs,
    ...
    # Create PCA (load precomputed EOFs if eof_path is given, else fit on training period)
    # ======================================================================================
    ...
    for key, values in data.items():
        ...
        shuffle = True if key == "train" else False
        dataloaders[key] = DataLoader(
            datasets[key], drop_last=True, batch_size=batch_size, num_workers=4, pin_memory=True, shuffle=shuffle
        )
