...
def make_args(ipython=False):
    ...
    if ipython:
        ...
        parser.add_argument("-horiz", "--train_horiz", default=16, type=int, help="Number of horizon datapoints.")
        parser.add_argument("-batch", "--batch_size", default=96, type=int, help="Batch size.")
        # Model
    ...
    if config["num_traindata"] is not None:
        config["lim_path"] = (
            PATH
            + f"/../../models/lim/cslim_{'-'.join(map(str, config['vars']))}/num_traindata/n_{config['num_traindata']}/cslim_hindcast_{'-'.join(map(str, config['vars']))}_eof{'-'.join(map(str, config['n_eof']))}"
        )
    else:
        config["lim_path"] = (
            PATH
            + f"/../../models/lim/cslim_{'-'.join(map(str, config['vars']))}/cslim_hindcast_{'-'.join(map(str, config['vars']))}_eof{'-'.join(map(str, config['n_eof']))}"
        )
...
for current_epoch in range(num_epochs):
    ...
    for lim_input, target, l in train_dataloader:
        ...
        context, _ = (
            l["month"].to(device, dtype=torch.long).split([train_horizon, unused], dim=-1)
            if config["film"]
            else (None, None)
        )
        # Prediction
    ...
    # 4. Print loss
    tend = time.time()
    print(f"Epoch {current_epoch}, train: {tl:.2e}, " + f"val.: {vl:.2e}, " + f"time: {tend - tstart}", flush=True)
...
if config["evaluate"]:
    ...
    perform_hindcast_evaluation(
        model, ds, dataloaders, "test", normalizer_pca, combined_eofa, lag_arr, model_path + "/metrics"
    )
# %%
