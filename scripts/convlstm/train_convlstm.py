...
def make_args(ipython=False):
    ...
    if ipython:
        ...
        # Data
        parser.add_argument("-vars", "--vars", nargs="+", default=["ssta", "ssha"], help="Variables to use.")
        parser.add_argument("-hist", "--hist", default=12, type=int, help="Length of history.")
        ...
        # Model
        parser.add_argument("-layers", "--num_layers", default=2, type=int, help="Number of LSTM layers.")
        parser.add_argument("-channels", "--num_channels", default=256, type=int, help="Latent dimenison.")
        parser.add_argument("-film", "--film", action="store_true", help="If set, conditioning with FiLM on month.")
        parser.add_argument("-members", "--members", default=16, type=int, help="Number of ensemble members.")
        # Training
        parser.add_argument(
            "-horiz", "--train_horiz", default=16, type=int, help="Number of horizon datapoints for training."
        )
        parser.add_argument("-loss", "--loss_type", default="crps", type=str, help="Loss type: 'mse' or 'crps'.")
        parser.add_argument(
        ...
        parser.add_argument(
            "-mlr", "--min_lr", default=1e-6, type=float, help="Minimum learning rate for Cosineannealing."
        )
...
class ConvLSTMTrainer(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.config["loss_type"] == "crps":
            ...
            self.loss_fn = lambda x, x_hat: (
                crps_class(x.unsqueeze(1), x_hat) + alpha * mse_class(x.unsqueeze(1), x_hat).mean(dim=1)
            )
        else:
    ...
    def _step(self, batch, batch_idx, history, horizon):
        ...
        context, _ = aux["month"].to(dtype=torch.long).split([history + horizon, unused], dim=-1)
...
wandb_logger = WandbLogger(project=config["wandb_project"], name=model_name, log_model=True)
