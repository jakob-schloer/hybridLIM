...
def make_args(ipython=False):
    ...
    if ipython:
        ...
        parser.add_argument("-dry","--dry", action="store_true",
                            help='If set, dry run.')
        parser.add_argument("-sweep","--sweep", action="store_true",
                            help='If set, run hyperparameter sweep.')
        # Save model
    ...
    for var in config['vars']:
        config['datapahts'][var] = path_to_data[var]
...
model_name = (f"{model_type}_"
              + f"eof_" + "-".join([str(n) for n in config['n_eof']])
...
for current_epoch in range(num_epochs):
    ...
    if model_path is not None:
        if current_epoch % 5 == 0:
            ...
            torch.save(checkpoint, model_path + f"/ckpt_epoch_{current_epoch:02d}.pt")
